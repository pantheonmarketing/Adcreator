...
import { defaultAdminRoles, defaultAppRoles, defaultPermissions, CreateRoleDto } from "@/modules/permissions/data/DefaultRoles";
...
import { createPermissions, createRolePermission } from "@/modules/permissions/services/PermissionsService";
...
async function seed() {
  ...
  const user1 = await createUser({ firstName: "John", lastName: "Doe", email: "john.doe@company.com", password: "password", admin: false });
  const user2 = await createUser({ firstName: "Luna", lastName: "Davis", email: "luna.davis@company.com", password: "password", admin: false });
...
async function createTenant(slug: string, name: string, users: { id: string; role: AppRoleEnum }[]) {
...
async function seedRolesAndPermissions(adminEmail?: string): Promise<void> {
  ...
  await Promise.all(
    users
      ...
      .map(async (user) => {
        ...
        adminRoles.map(async (adminRoleId) => {
    ...
    tenants.map(async (tenant) => {
      tenant.users.map(async (tenantUser) => {
        appRoles.map(async (appRoleId) => {
          ...
          return await createUserRole({ userId: tenantUser.userId, roleId: appRoleId, tenantId: tenant.id });
        });
      });
    })
  );
  ...
  if (guestRole) {
    const viewAndReadPermissions = (await db.permission.getAll()).filter((f) => f.name.includes(".view") || f.name.includes(".read"));
    await Promise.all(
