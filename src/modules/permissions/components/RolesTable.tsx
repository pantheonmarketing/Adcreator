...
export default function RolesTable({ items, canUpdate = true, tenantId = null }: Props) {
  ...
  const [headers, setHeaders] = useState<RowHeaderDisplayDto<RoleWithPermissionsAndUsersDto>[]>([]);

  const [permissionsModalOpen, setPermissionsModalOpen] = useState(false);
  const [selectedRole, setSelectedRow] = useState<RoleWithPermissionsAndUsersDto>();
  ...
  useEffect(() => {
    ...
    const headers: RowHeaderDisplayDto<RoleWithPermissionsAndUsersDto>[] = [
      {
        name: "name",
    ...
    if (!tenantId) {
      headers.push({
        ...
        value: (i) => (i.assignToNewUsers ? <CheckIcon className="h-4 w-4 text-teal-500" /> : <XIcon className="h-4 w-4 text-gray-300" />),
      });
    }
    setHeaders(headers);
