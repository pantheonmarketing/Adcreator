    ...
    <Fragment>
      {isMonthlyOrYearly ? (
        <div className={clsx("flex items-center justify-center space-x-4", className)}>
          ...
          >
            <div className={clsx("rounded-full bg-primary shadow-md outline-none transition", size === "sm" && "h-4 w-8", size === "md" && "h-8 w-16")}></div>
            <div
              className={clsx(
                "absolute left-1 top-1 inline-flex transform items-center justify-center rounded-full bg-primary-foreground shadow-sm transition-all duration-200 ease-in-out",
                size === "sm" && "h-2 w-2",
                size === "md" && "h-6 w-6",
        ...
        <Fragment>
          <div className={className}>
            <select
              value={billingPeriod}
              onChange={(e) => setBillingPeriod(Number(e.currentTarget.value) as SubscriptionBillingPeriod)}
              className={clsx(
                "block w-full rounded-md border-gray-300 py-2 pl-3 pr-10 text-base text-gray-800 focus:border-primary focus:outline-none focus:ring-primary sm:text-sm",
