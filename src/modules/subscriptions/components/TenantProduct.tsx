...
import { TenantSubscriptionProductPriceModel } from "@/db/models";
...
export default function TenantProduct({ item, onCancel }: Props) {
  ...
  function getFormattedPrice(price: TenantSubscriptionProductPriceModel & { subscriptionPrice: SubscriptionPriceDto | null }) {
    const formattedPrice = PricingUtils.getFormattedPriceInCurrency({
      price: Number(price.subscriptionPrice?.price),
      currency: price.subscriptionPrice?.currency,
    });
    let priceString = ``;
                          ...
                          <div key={idx} className="flex items-center space-x-1">
                            ...
                            {tier.perUnitPrice && (
                              <div>
                                <>
                                  +
                                  {PricingUtils.getFormattedPriceInCurrency({
                            ...
                            {tier.flatFeePrice && (
                              <div>
                                <>
                                  +
                                  {PricingUtils.getFormattedPriceInCurrency({
                                    price: Number(tier.flatFeePrice),
                                    currency: price.subscriptionUsageBasedPrice?.currency,
                                  })}
                                </>
                              </div>
