...
export async function createPlan(
  ...
  // Save to db
  const productId = await db.subscriptionProduct.createSubscriptionProduct({
    ...
    badge: plan.badge,
    groupTitle: plan.groupTitle || null,
  ...
  const product = await db.subscriptionProduct.getSubscriptionProduct(productId);
  ...
  await Promise.all(
    prices.map(async (price) => {
      ...
      // Save to db
      return await db.subscriptionProduct.createSubscriptionPrice({
        ...price,
  ...
  if (usageBasedPrices) {
    await Promise.all(
      usageBasedPrices.map(async (usageBasedPrice) => {
        ...
        const createdPriceId = await db.subscriptionProduct.createSubscriptionUsageBasedPrice({
          subscriptionProductId: product.id!,
          stripeId: stripePrice?.id ?? "",
        ...
        const createdPrice = await db.subscriptionProduct.getSubscriptionUsageBasedPrice(createdPriceId);
        if (!createdPrice) {
          throw new Error("Could not create usage based price");
        }
        await Promise.all(
          usageBasedPrice.tiers.map(async (tierPrice) => {
            await db.subscriptionProduct.createSubscriptionUsageBasedTier({
...
export async function syncPlan(
  ...
  await db.subscriptionProduct.updateSubscriptionProductStripeId(plan.id, {
    stripeId: stripeProduct.id,
  });
  ...
  prices.map(async (price) => {
    ...
    // Save to db
    await db.subscriptionProduct.updateSubscriptionPriceStripeId(price.id ?? "", {
      stripeId: stripePrice?.id ?? "",
    });
  });
}
...
export async function updatePlan(plan: SubscriptionProductDto, features: SubscriptionFeatureDto[]) {
  ...
  await db.subscriptionProduct.updateSubscriptionProduct(plan.id, {
    order: plan.order,
    title: plan.title,
    model: plan.model,
...
export async function deletePlan(plan: SubscriptionProductDto) {
  ...
  await Promise.all(
    plan.prices
      ...
      .map(async (price) => {
        ...
        if (price.id) {
          await db.subscriptionProduct.deleteSubscriptionPrice(price.id);
        }
      ...
      plan.usageBasedPrices?.map(async (price) => {
        ...
        await Promise.all(
          price.tiers.map(async (tier) => {
            return db.subscriptionProduct.deleteSubscriptionUsageBasedTier(tier.id);
          })
        );
        await db.subscriptionProduct.deleteSubscriptionUsageBasedPrice(price.id);
  ...
  if (plan.id) {
    await db.subscriptionProduct.deleteSubscriptionProduct(plan.id);
  }
}
...
export async function getAcquiredItemsFromCheckoutSession(session_id: string | null): Promise<CheckoutSessionResponse | null> {
  ...
  try {
    ...
    await Promise.all(
      line_items.map(async (line_item) => {
        const flatPrice = await db.subscriptionProduct.getSubscriptionPriceByStripeId(line_item.price.id);
        const usageBasedPrice = await db.subscriptionProduct.getSubscriptionUsageBasedPriceByStripeId(line_item.price.id);
  ...
  await Promise.all(
    products.map(async (product) => {
      const subscriptionProduct = await db.subscriptionProduct.getSubscriptionProduct(product.id);
...
export async function autosubscribeToTrialOrFreePlan({ tenantId }: { tenantId: string }) {
  ...
  const allProducts = await db.subscriptionProduct.getAllSubscriptionProducts(true);
