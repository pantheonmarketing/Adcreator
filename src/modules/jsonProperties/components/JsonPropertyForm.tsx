      ...
      <Form onSubmit={onSubmit}>
        <div className="mb-4 space-y-2">
          <div>
            ...
            <Input ref={mainInput} name="title" title="Title" value={title} onChange={(e) => setTitle(e.currentTarget.value)} required />
          </div>
          <div>
            <label className="mb-1 text-xs font-medium">
              Name <span className="text-red-500">*</span>
            </label>
            <Input name="name" title="Name" value={name} onChange={(e) => setName(e.currentTarget.value)} required />
          </div>
          <div>
            <div>
              <label className="mb-1 text-xs font-medium">
                {t("models.jsonProperty.type")} <span className="text-red-500">*</span>
              </label>
              <InputSelect
                name="type"
                value={type}
                onChange={(e) => setType(e as JsonPropertyType)}
          ...
          {(type === "select" || type === "multiselect") && (
            <div className="space-y-1">
              <label className="mb-1 text-xs font-medium">Options</label>
              <div>
                <TableSimple
                  items={options}
                  actions={[
                    ...
                    },
                    {
                      ...
                      renderTitle: (_) => <TrashIcon className="h-4 w-4 text-muted-foreground group-hover:text-foreground" />,
                    },
                  ]}
                  headers={[
                    {
          ...
          <div>
            <InputCheckbox name="required" title="Required" value={isRequired} onChange={setIsRequired} />
          </div>
        </div>
  ...
  return (
    <Form onSubmit={onSubmit} className="space-y-2">
      <div>
        ...
        <Input ref={mainInput} name="name" title="Name" value={name} onChange={(e) => setName(e.currentTarget.value)} required />
      </div>
      <div>
        <label className="mb-1 text-xs font-medium">
          Value <span className="text-red-500">*</span>
        </label>
        <Input name="value" title="Value" value={value} onChange={(e) => setValue(e.currentTarget.value)} required />
      </div>
