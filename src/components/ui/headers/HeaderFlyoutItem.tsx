    ...
    <Popover className="relative">
      {({ open }) => (
        <>
          ...
          >
            <span>{title}</span>
            <ChevronDownIcon className={clsx(open ? "" : "text-muted-foreground", "h-5 w-5 group-hover:text-muted-foreground")} aria-hidden="true" />
          </Popover.Button>
          ...
          >
            <Popover.Panel className="absolute left-1/2 z-20 mt-3 w-screen max-w-xs -translate-x-1/2 transform px-2 sm:px-0">
              {({ close }) => (
                <div className="overflow-hidden rounded-lg border border-border bg-background shadow-lg ring-opacity-5">
