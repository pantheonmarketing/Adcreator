...
export default function TablePagination({ page, pageSize, totalItems, totalPages }: Props) {
  ...
  useEffect(() => {
    const pageSizeParam = searchParams.get("pageSize")?.toString() ?? DEFAULT_PAGE_SIZE.toString();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-2">
        ...
        >
          {[undefined, ...PAGE_SIZE_OPTIONS].map((f, idx) => {
            return (
              <option key={idx} value={Number(f ?? DEFAULT_PAGE_SIZE)} className="lowercase">
                {f === undefined ? DEFAULT_PAGE_SIZE : f} {t("shared.perPage")?.toLowerCase()}
              </option>
            );
          })}
        </select>
        <div className="inline-flex items-center justify-center gap-2">
          ...
          {showPageNumberModal ? (
            <input
              ...
              value={state.page}
              onChange={(e) => setState({ ...state, page: parseInt(e.currentTarget.value) })}
...
function PageOptionsForm({
  ...
  useEffect(() => {
    if (totalItems) {
      setTotalPages(Math.ceil(totalItems / (state.pageSize || DEFAULT_PAGE_SIZE)));
    }
  ...
  function existingChanges() {
    return state.pageSize !== DEFAULT_PAGE_SIZE || state.currentPage !== 1;
  }
  return (
    <form onSubmit={handleSubmit} className="space-y-2">
      <div>
        <div>
          <label className="mb-1 text-xs font-medium">{t("shared.pageSize")}</label>
          <InputSelect
            value={state.pageSize || "default"}
            onChange={(e) => setState({ ...state, pageSize: Number(e) })}
            options={["default", ...PAGE_SIZE_OPTIONS].map((size) => {
              return {
                value: size,
                name: size === "default" ? t("shared.default") : size.toString(),
              };
            })}
          />
        </div>
      ...
      <div>
        <InputNumber
          ...
          min={1}
          max={totalPages}
          onChange={(e) => setState({ ...state, currentPage: Number(e) })}
        />
      </div>
