    ...
    <IndexPageLayout
      ...
      buttons={
        <>
          ...
          <ButtonPrimary disabled={!getUserHasPermission(adminData, "admin.accounts.create")} onClick={() => setCreatingNewAccount(true)}>
            {t("shared.new")}
          </ButtonPrimary>
        </>
      }
    >
      <TenantsTable items={data.items} pagination={data.pagination} tenantInvoices={data.tenantInvoices} isStripeTest={data.isStripeTest} />
    ...
    <form action={action} className="py-1">
      ...
      <div className="space-y-3">
        <div>
          ...
          <Input ref={firstInput} autoFocus name="name" title={t("shared.name")} value={name} onChange={(e) => setName(e.currentTarget.value)} required />
        </div>
        <div>
          <label className="mb-1 text-xs font-medium">
