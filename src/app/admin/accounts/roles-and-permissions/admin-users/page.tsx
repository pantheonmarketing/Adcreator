...
export const actionAdminRolesAdminUsers = async (prev: any, form: FormData) => {
  ...
  if (action === "edit") {
    ...
    if (add) {
      await createUserRole({ userId, roleId, tenantId: null });
    } else {
      const role = await db.role.get(roleId);
      if (role?.name === AdminRoleEnum.SuperAdmin && userInfo.userId === userId) {
        return { error: "You can't remove Super Admin role from yourself" };
      }
      await deleteUserRole({ userId, roleId, tenantId: null });
    }
