...
import { RoleWithPermissionsDto, TenantModel, TenantWithDetailsDto, UserWithRolesDto } from "@/db/models";
import { db } from "@/db";
...
export const actionAdminAccountUsersAccount = async (prev: any, form: FormData) => {
  ...
  if (action === "edit") {
    ...
    if (add) {
      await createUserRole({ userId, roleId, tenantId: tenantId });
    } else {
      await deleteUserRole({ userId, roleId, tenantId: tenantId });
    }
